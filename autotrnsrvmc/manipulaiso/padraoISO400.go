...
func Padraoiso400(iso_padrao []byte) ([]byte, error) {

	// Inicia registros de hora
	data := time.Now()
	...
	fmt.Printf("\n MENSAGEM RECEBIDA EBCDIC RETORNO CANCELAMENTO:\n")
	fmt.Printf("% x\n", b)
	...
	pnew_message.Field(38, data.Format(("150405"))) //codigo de resposta
	...
	//	panic(err)
	//}
	//pnew_message.Field(48, de48) //dados adicionais
	////
	...
	//}
	if cod_auto == "" {
		pnew_message.Field(60, "C@    CANC MASTERCARD - Via Cliente    @TESTEPAY - TESTE111111 E            @RUA XXXX XXXXX C                 SP BR@CNPJ:01000000000100                   @TID: GT000131@EC:XXXXXXXXX@                    @VENDA CREDITO A VISTA                 @************0045   @19/06/24                         09:10@VALOR APROVADO:               R$ 11,00@CV:XXXXXXXXXXXX            AUTO:000000@DOC:000000@TERM:XXXXXXXX@@               ")
		pnew_message.Field(62, "C@    CANC MASTERCARD - Via Cliente    @TESTEPAY - TESTE111111 E            @RUA XXXX XXXXX C                 SP BR@CNPJ:01000000000100                   @TID: GT000131@EC:XXXXXXXXX@                    @VENDA CREDITO A VISTA                 @************0045   @19/06/24                         09:10@VALOR APROVADO:               R$ 11,00@CV:XXXXXXXXXXXX            AUTO:000000@DOC:000000@TERM:XXXXXXXX@@ MEDIANTE A SENHA               ")
	}
	////
	de90, err := p_message.GetString(90)
	if err != nil {
		panic(err)
	}
	fmt.Println("PRINT DO tid", tid)
	fmt.Println("PRINT DO moeda", moeda)
	fmt.Println("PRINT DO 90", de90)
	if de90 != "" {
		pnew_message.Field(90, de90) //dados adicionais
	} else {
		pnew_message.Field(90, "000000000000000000000000000000000000000000") //dados adicionais
	}
	////
